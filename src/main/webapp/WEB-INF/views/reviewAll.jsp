	...
	<tr align="center">
		...
		<td>${allReview.CREATEDATE}</td>
					...
					<!-- 전체리뷰목록 -> 인덱스10 간격으로 버튼 만들어주기. --> 
 					<c:forEach items="${allReviewPage}" var="allReview" step="10" varStatus="status">
						 <button onclick="page(${allReview.REVIEWNUM})">${status.count}Page</button>
						<%-- <button onclick="page(${status.index})">${status.count}Page(${status.index})</button> --%>
					</c:forEach>
